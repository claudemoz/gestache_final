      ...
      <el-form label-position="top" label-width="100px" >
        ...
        <el-form-item label="Confirmation du mot de passe">
          <BaseInput
            ...
            type="password"
            id="passwordConfirm"
            area-describeby="pwdConfirmError"
            :area-invalid="v$.passwordConfirm.$invalid"
          ></BaseInput>
          <el-alert
        ...
        <el-form-item label="Confirmation du mot de passe">
          <BaseCheckBox 
            v-model="condition"
            area-describeby="condition"
            :area-invalid="v$.condition.$invalid"
          >
            Conditions d'utilisation
          </BaseCheckBox>
          <!-- <el-checkbox v-model="condition"></el-checkbox> -->
          <el-alert
            v-if="v$.condition.$error"
            :title="v$.condition.$errors[0].$message"
            id="condition"
            type="error"
            :closable="false"
          ></el-alert>
  ...
  setup() {
    ...
    const sendForm = async ()=>{
      ...
      state.apiError = null
        if(!v$.value.$error){
          state.loading = true
        const res  = await store.dispatch('USERS_MODULE/register',{ email: state.email, password: state.password})
        if(res == true){
